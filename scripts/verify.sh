...
EOF
    ...
    check "the repository offers this profile packages" bash -c \
      "pacman -Sl omarchy-server | awk \"{print \\\$2}\" | sort >/tmp/list; for p in omarchy-server omarchy-server-keyring omarchy-server-settings; do grep -qx \"\$p\" /tmp/list || exit 1; done"

    # SigLevel = Required is in force for both of these; an unsigned or badly
    # signed package would stop the transaction here.
    check "omarchy-server-settings installs from the remote repository" bash -c \
      "pacman -S --noconfirm omarchy-server-settings && test -d /usr/share/omarchy/default"
    ...
    check "the installed packages report the repository they came from" bash -c \
      "pacman -Qi omarchy-server | grep -qE \"^Version +: 4\\.0\\.1-\" && pacman -Qi omarchy-server-settings | grep -qE \"^Version +: 4\\.0\\.1-\""
    ...
    # The runtime package is the guinea pig: nothing else in the repository
    # depends on it, so it can be removed and reinstalled without dragging the
    # rest of the transaction along.
    tampered=$(ls /srv-tampered/omarchy-server-[0-9]*.pkg.tar.zst)
    rm -f "$tampered.sig"
    ...
    # -R, not -Rns: its dependencies came from the same repository and are
    # what the reinstall below needs to be in place.
    pacman -R --noconfirm omarchy-server >/dev/null
    ...
    rm -f /var/cache/pacman/pkg/omarchy-server-[0-9]*
