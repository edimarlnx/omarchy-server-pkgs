...
#
# Sources: the PKGBUILDs pull the pinned commit over https by default. A local
# checkout can be substituted by exporting OMARCHY_SRC, which is what the
# omarchy-server lab repository's pkgs/build.sh does.
